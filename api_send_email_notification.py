...
# -----------------------
# Trigger Emails (Single Lead Processing)
# -----------------------
def trigger_daily_job_emails():
    leads = get_active_job_board_leads()

    if not leads:
        print("ℹ️ No active leads found.")
        return {
            "success": True,
            "total_leads": 0,
            "triggered_count": 0,
    ...
    print(f"🚀 Found {len(leads)} leads. Starting process...\n")
    ...
    for index, lead in enumerate(leads, start=1):
        ...
        print(f"➡️ [{index}/{len(leads)}] Processing: {name} ({apw_id})")

        if not apw_id:
            print(f"⚠️ Skipped: Missing apwId\n")
            skipped.append({"name": name, "reason": "NO_APW_ID"})
            continue
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                triggered.append({"name": name, "apw_id": apw_id})

            # 🚫 NOT FOUND (invalid apwId)
            elif response.status_code == 404:
                print(f"🚫 Lead not found in Django\n")
                skipped.append({"name": name, "apw_id": apw_id, "reason": "NOT_FOUND"})

            # ⏭️ NOT ENOUGH JOBS
            elif response.status_code == 422:
                print(f"⏭️ Skipped (Not enough jobs today)\n")
                skipped.append({"name": name, "apw_id": apw_id, "reason": "NO_JOBS"})
        ...
        # 🔥 IMPORTANT: 6 seconds delay (your requirement)
    ...
    print(f"Emails Sent     : {len(triggered)}")
...
# -----------------------
if __name__ == "__main__":
    print("🚀 Starting Daily Job Email Trigger...\n")
    try:
        result = trigger_daily_job_emails()
        print("✅ Done!")
    except Exception as e:
